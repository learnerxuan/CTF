...
**Search strings:**
```python
mcp__ghidra__search_strings(pattern="flag")
```

**Get function details:**
```python
mcp__ghidra__get_function_details(address="0x00101229")
```

**Find cross-references:**
```python
mcp__ghidra__get_xrefs_to(address="0x00101229")
mcp__ghidra__get_xrefs_from(address="0x001019b0")
```

**Get call graph:**
```python
mcp__ghidra__get_call_graph(address="0x001019b0")
```
...
**Start debugging:**
```python
mcp__pwndbg__pwndbg_start(
    binary_path="./chorus_of_splinters"
)
```

**Check security:**
```python
mcp__pwndbg__pwndbg_checksec()
```

**Get PIE base:**
```python
mcp__pwndbg__pwndbg_piebase()
```

**View memory map:**
```python
mcp__pwndbg__pwndbg_vmmap()
```

**Set breakpoint:**
```python
mcp__pwndbg__pwndbg_breakpoint(location="main")
```

**Run program:**
```python
mcp__pwndbg__pwndbg_run()
```

**Continue execution:**
```python
mcp__pwndbg__pwndbg_continue()
```

**Step instruction:**
```python
mcp__pwndbg__pwndbg_stepi(count=1)
```

**View registers:**
```python
mcp__pwndbg__pwndbg_registers()
```

**Examine memory:**
```python
mcp__pwndbg__pwndbg_examine(
    address="0x555555558000",
    count=10,
    format="gx"  # Giant words (8 bytes) in hex
)
```

**View GOT:**
```python
mcp__pwndbg__pwndbg_got()
```

**View heap:**
```python
mcp__pwndbg__pwndbg_heap()
mcp__pwndbg__pwndbg_bins()
```

**Execute custom command:**
```python
mcp__pwndbg__pwndbg_execute(command="p &notes")
```
